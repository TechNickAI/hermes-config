...
12. **`pm2 startup` needs sudo and bakes in the wrong `--hp` under Hermes.** Reboot
    ...
    `--hp`:
    `HOME=/Users/<user> PM2_HOME=/Users/<user>/.pm2 pm2 startup launchd -u <user> --hp /Users/<user>`.
...
## Exposing the router on a real custom domain (Cloudflare Tunnel)

Tailscale Funnel only serves a TLS cert for the host's `*.ts.net` name, so a plain
`CNAME yourname.example.com → host.tailnet.ts.net` gives a **cert-name mismatch**
(browser sends SNI for your domain, funnel presents a ts.net cert → TLS fails). To put a
real domain on the router, use a **Cloudflare Tunnel** into the same Caddy loopback.
It's the easy path when the zone is already on Cloudflare DNS: Cloudflare terminates
HTTPS (auto-issues + renews the cert) and `cloudflared` tunnels outbound to
`127.0.0.1:8080` — no public IP, no port-forward, no inbound firewall ports.

**Shortcut:** if `~/.cloudflared/cert.pem` already exists (a prior
`cloudflared tunnel login`), the whole thing is headless — no dashboard token, no
browser click needed. The CLI path becomes the _lower-effort_ route. Check first:
`ls ~/.cloudflared/cert.pem`.

```bash
# 0. one-time only if no cert.pem yet (opens browser to pick the zone):
...
# 3. create the proxied DNS record automatically + validate
cloudflared tunnel route dns <name> sub.example.com
...
**Verify with a real external round-trip** (through Cloudflare, not loopback):
`curl -sS -o /dev/null -w "HTTP %{http_code}\n" https://sub.example.com/health` → 200,
and `curl -sv https://sub.example.com/health 2>&1 | grep -iE "subject:|issuer:"` should
show `CN=<your-domain>` issued by Let's Encrypt (Cloudflare). PM2 logs should show 4
`Registered tunnel connection` lines (redundant QUIC edges).

**Pitfalls:**

- The custom domain fronts the **same Caddy** as the funnel — so `/`, `/hooks/*`, and
  every app path become **publicly reachable** on the pretty URL. Gate anything
  sensitive (auth sidecar) or scope the Caddy routes; don't assume the domain is
  private.
- **Keep the Tailscale Funnel** — it's an independent door to the same Caddy, not a
  conflict. Both proxy into `127.0.0.1:8080`; no route duplication.
- `cloudflared` is outbound-only; a stale version just logs an update warning (harmless)
  — `brew upgrade cloudflared` when convenient.
- Rollback is clean and touches nothing else: `pm2 delete cloudflared && pm2 save`,
  delete the DNS record in the CF UI, `cloudflared tunnel delete <name>`, `rm` the
  config + creds json.
