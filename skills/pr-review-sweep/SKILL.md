...
- Claude Code CLI installed and an ai-coding-config plugin with an
  `address-pr-comments` skill (the agent your sub-process actually runs)
...
2. **For each PR, fetch comments from both endpoints:**
   ...
   "Unhandled" = not from PR author AND zero reactions. No bot allowlist — catches
   bot reviewers (CodeRabbit, claude-review) and humans alike.
...
4. **For each remaining PR (serial, not parallel):**
   - Provision a fresh workshop: `workshop {repo} pr-sweep-{pr}` (or
     `git clone` + `git checkout -b pr-sweep-{pr}`)
...
This skill is provided by the ai-coding-config plugin (globally installed). It knows
the project's triage conventions and review standards. Do NOT manually triage — you
MUST use the Skill tool to load and execute the address-pr-comments skill.

If the skill creates fixes, they go in a new follow-up PR with labels: {FOLLOWUP_LABELS}.

After creating any follow-up PR, use the Skill tool to invoke address-pr-comments on
the NEW PR number too (cascading sweep).
...
""",
   ...
   - **Verify completion:** When the sub-process returns, scan its output. If it did
     NOT use the Skill tool for `address-pr-comments`, send it back with:

     > "You did not use the Skill tool. Invoke the address-pr-comments skill now using
     > the Skill tool."

     Keep doing this until the skill is actually invoked. (Without this, Claude Code
     in ACP mode will silently freelance instead of running the skill — `/slash`
     prefixes are NOT auto-intercepted in ACP/--print mode.)
...
- **Serial, not parallel** — Claude Code workshops grab the same `~/dev/{repo}*`
  paths; parallel runs collide. Serial keeps state clean.
...
Ported from an OpenClaw `workflows/pr-review-sweep/` folder (AGENT.md + config.md)
into a single Hermes skill. The config.md knobs are now the table at the top of this
file; the AGENT.md procedure is the body. Cron scheduling lives separately in
`hermes cron`, where it belongs.
